...
app.controller('projectCtrl', function($scope, $filter, $http) {
  $scope.getApps =
    ...
    $http.get("config/apps.json").then(function(response) {
      ...
      $scope.getWebs =
          function() {
    $scope.isNotLoaded = true;
    $http.get("config/web.json").then(function(response) {
      $scope.isNotLoaded = false;
      $scope.projects = response.data;
      $scope.projects.title = "Web Applications";
          ...
          $scope.getGames =
              function() {
    $scope.isNotLoaded = true;
    $http.get("config/games.json").then(function(response) {
      $scope.isNotLoaded = false;
      $scope.projects = response.data;
    ...
    $scope.isNotLoaded = true;
    $http
        .get("https://api.github.com/users/nuwan94/repos?page=1&per_page=100", {
          headers : {
...
app.controller('resumeCtrl', function($scope, $filter, $http) {
  $scope.showWork =
      function() {
    $scope.isNotLoaded = true;
    $http.get("config/work.json").then(function(response) {
      $scope.isNotLoaded = false;
      ...
      $scope.showEdu =
          function() {
    $scope.isNotLoaded = true;
    $http.get("config/edu.json").then(function(response) {
      $scope.isNotLoaded = false;
      $scope.resu = response.data;
      $scope.resu.title = "Educational Quaification";
          ...
          $scope.showComm =
              function() {
    $scope.isNotLoaded = true;
    $http.get("config/comm.json").then(function(response) {
      $scope.isNotLoaded = false;
      $scope.resu = response.data;
      $scope.resu.title = "Communities & Societies";
              ...
              $scope.showCont =
                  function() {
    $scope.isNotLoaded = true;
    $http.get("config/cont.json").then(function(response) {
      $scope.isNotLoaded = false;
      $scope.resu = response.data;
      $scope.resu.title = "Contests & Hackathons";
                  ...
                  $scope.showSkill =
                      function() {
    $scope.isNotLoaded = true;
    $http.get("config/skill.json").then(function(response) {
      $scope.isNotLoaded = false;
      $scope.resu = response.data;
    ...
    $scope.isNotLoaded = true;
    $http.get("config/train.json").then(function(response) {
      $scope.isNotLoaded = false;
      $scope.resu = response.data;
...
app.controller('statCtrl', function($scope, $filter, $http) {
  ...
  $scope.getStat = function() {
    $http
        .get(
            "https://api.github.com/users/nuwan94?client_id=825bdda0c89053fcd4ed&client_secret=fd737cee6549af66d686b4ab682e0bbca6862035")
        .then(function(response) {
    ...
    // $http.get("https://api.stackexchange.com/2.2/users/3125964?order=desc&sort=reputation&site=stackoverflow").then(function
    // (response) { 	if (response.data.items[0].reputation) { 		$scope.so =
